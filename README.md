...
- **一鍵複製修復清單**：將問題匯出為 JSON 格式，直接貼給 AI 工具修復
...
分析完成後，點擊「📋 本大小姐賜你修復清單」按鈕，即可複製 JSON 格式的問題清單：
  ...
  "issues": [
    {
      ...
      "name": "網頁標題 (title)",
      "value": "短標題",
      "suggestion": "建議標題長度 30-60 字元"
    },
    {
      "key": "og_image",
      "name": "Open Graph 圖片",
      "value": null,
      "suggestion": "請新增 <meta property=\"og:image\" content=\"圖片完整網址\">，建議尺寸 1200x630"
    }
  ]
}
```

直接貼給 Cursor、Claude Code 等 AI 工具，讓它幫你修復 SEO 問題。
...
| 項目 | 說明 | 優先度 |
|------|------|--------|
| HTTP 安全標頭 | 加入 CSP、X-Frame-Options、HSTS | 高 |
| HTML 大小限制 | 防止巨大 HTML 造成記憶體問題 | 高 |
| Redis Rate Limiter | 記憶體版不適合多 worker 部署 | 中 |
| ProxyFix | 正確處理 X-Forwarded-For | 中 |
| CORS 設定 | 未來 API 化需要 | 低 |
...
Google 雖然有公佈一些官方指南和已確認的排名因素，但完整的演算法細節和各因素的權重從未公開。
這些 SEO 知識都是使用者根據 Google 的官方文件，或者是一些前人的總結、一些觀察、一些可能的趨勢推敲出來的。
我們只是發現「好像這樣做的網站排名比較前面」，然後把它整理成一個又一個的 SEO 檢查清單。
